...
//          println(s"antinodes $antinode1, $antinode2")
  ...
  def part1(arr: Array[Array[Char]]): Int = {
    ...
    val antinodes = antennas
      .map { case (letter, list) =>
        (letter, countAntinodes1(list, maxI, maxJ))
      }
    ...
    for (i <- antennas.indices) {
      if (i + 1 < antennas.length) {
        for (j <- antennas.indices.drop(i + 1)) {
          ...
          var antinode1 = (antenna1._1 + deltaI, antenna1._2 + deltaJ)
          while (isInside(antinode1._1, antinode1._2, maxI, maxJ)) {
            result = antinode1 :: result
            antinode1 = (antinode1._1 + deltaI, antinode1._2 + deltaJ)
          }

          var antinode2 = (antenna2._1 - deltaI, antenna2._2 - deltaJ)
          while (isInside(antinode2._1, antinode2._2, maxI, maxJ)) {
            result = antinode2 :: result
            antinode2 = (antinode2._1 - deltaI, antinode2._2 - deltaJ)
          }
        }
      }
    }
  ...
  def part2(arr: Array[Array[Char]]): Int = {
    val antennas: Map[Char, List[(Int, Int)]] = findAntennas(arr)
    val maxI = arr.length
    val maxJ = arr(0).length
    val antinodes = antennas
      .map { case (letter, list) =>
        (letter, countAntinodes2(list, maxI, maxJ))
      }
    val notOnlyAntennas = antennas.filter { case (_, list) =>
      list.length > 1
    }
    (antinodes.values.flatten ++ notOnlyAntennas.values.flatten).toSet.size
  }
}
